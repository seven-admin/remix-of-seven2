...
import { Plus, Trash2, Settings2, Sparkles, Loader2, Save, AlertTriangle, Info, Wand2 } from 'lucide-react';
...
interface NegociacaoCondicoesPagamentoInlineEditorProps {
  negociacaoId: string;
...
export function NegociacaoCondicoesPagamentoInlineEditor({ 
  negociacaoId,
  empreendimentoId,
  valorReferencia = 0,
  readonly = false,
  ...
  const [editingForAdvanced, setEditingForAdvanced] = useState<NegociacaoCondicaoPagamento | null>(null);
  ...
  const { totalConfigurado, percentualConfigurado, diferenca, diferencaCents, alertasComerciais } = useMemo(() => {
    let totalCents = 0;
    const valorRefCents = toCents(valorReferencia);
    const todasCondicoes: Array<{ tipo_parcela_codigo: string; quantidade: number; valor: number }> = [];
    ...
    condicoes.forEach(c => {
      ...
      todasCondicoes.push({
        tipo_parcela_codigo: edited?.tipo_parcela_codigo ?? c.tipo_parcela_codigo,
        quantidade: qtd,
        valor,
      });
    });
    ...
    // Validar contra configuração comercial
    const alertas = validarCondicoesVsConfiguracao(todasCondicoes, valorReferencia, configComercial || null);
    ...
    return { 
      ...
      alertasComerciais: alertas 
    };
  }, [condicoes, editedConditions, newConditions, valorReferencia, configComercial]);
  ...
  const confirmDelete = () => {
    if (conditionToDelete) {
      deleteCondicao.mutate({ id: conditionToDelete, negociacaoId });
  ...
  const openAdvancedForm = (condition?: NegociacaoCondicaoPagamento) => {
        ...
        if (Object.keys(changes).length > 0) {
          ...
          if (original) {
            await updateCondicao.mutateAsync({
              id,
              negociacaoId,
              tipo_parcela_codigo: changes.tipo_parcela_codigo ?? original.tipo_parcela_codigo as TipoParcelaCodigo,
              quantidade: changes.quantidade ?? original.quantidade ?? 1,
              valor: changes.valor ?? original.valor ?? 0,
              forma_pagamento: changes.forma_pagamento ?? original.forma_pagamento as FormaPagamento ?? 'boleto',
              forma_quitacao: changes.forma_quitacao ?? original.forma_quitacao as FormaQuitacao ?? 'dinheiro',
            });
          }
        }
      }
  ...
  const getDisplayValue = (condition: NegociacaoCondicaoPagamento, field: keyof EditedCondition) => {
    return editedConditions[condition.id]?.[field] ?? condition[field as keyof NegociacaoCondicaoPagamento];
  };
  ...
  return (
    <div className="space-y-4">
      ...
      {valorReferencia > 0 && diferencaCents !== 0 && !hasChanges && (
        <Alert variant={isCentsDifference ? "default" : "destructive"} className={cn("border-2", isCentsDifference && "border-amber-300 bg-amber-50 dark:bg-amber-950/20")}>
          ...
          <AlertDescription className={cn("mt-1", isCentsDifference && "text-amber-700 dark:text-amber-300")}>
            {diferenca > 0 ? (
              <>
                O total configurado é <strong>{formatCurrency(Math.abs(diferenca))}</strong> menor que o valor da proposta.
              </>
            ) : (
              <>
                O total configurado é <strong>{formatCurrency(Math.abs(diferenca))}</strong> maior que o valor da proposta.
              </>
            )}
            {isCentsDifference ? (
      ...
      {/* Alertas de limites comerciais */}
      {alertasComerciais.length > 0 && (
        <Alert variant="default" className="border-amber-300 bg-amber-50 dark:bg-amber-950/20">
          ...
          <AlertTitle className="font-semibold text-amber-800 dark:text-amber-200">
            Atenção aos limites comerciais
          </AlertTitle>
          <AlertDescription className="mt-1 space-y-1">
            {alertasComerciais.map((alerta, index) => (
              <div key={index} className="text-amber-700 dark:text-amber-300 text-sm">
                • {alerta.mensagem}
              </div>
            ))}
          </AlertDescription>
          ...
          <Tooltip>
            <TooltipTrigger asChild>
              <div className="flex items-center gap-2 text-xs text-muted-foreground cursor-help">
                ...
                <span>Limites: Entrada mín. {configComercial.entrada_minima || 0}% | Parcelas mensais máx. {configComercial.max_parcelas_mensais || '-'}</span>
              </div>
            </TooltipTrigger>
            <TooltipContent side="bottom" className="max-w-xs">
              <div className="space-y-1 text-xs">
                <p><strong>Configuração Comercial do Empreendimento:</strong></p>
                <p>• Entrada mínima: {configComercial.entrada_minima || 0}%</p>
                <p>• Máx. parcelas entrada: {configComercial.max_parcelas_entrada || 'Sem limite'}</p>
                <p>• Máx. parcelas mensais: {configComercial.max_parcelas_mensais || 'Sem limite'}</p>
                <p>• Máx. parcelas anuais: {configComercial.limite_parcelas_anuais || 'Sem limite'}</p>
                <p>• Índice reajuste: {configComercial.indice_reajuste || 'INCC'}</p>
              </div>
            </TooltipContent>
          </Tooltip>
        </TooltipProvider>
              ...
              {condicoes.length === 0 && newConditions.length === 0 ? (
                ...
                <>
                  ...
                  {condicoes.map((condicao, index) => {
                    const qtd = (getDisplayValue(condicao, 'quantidade') as number) ?? 1;
                  ...
                  {newConditions.map((newCond, index) => {
                    const total = (newCond.quantidade || 0) * (newCond.valor || 0);
              ...
              <div className="flex items-center gap-6 text-sm">
                ...
                {valorReferencia > 0 && (
                  <div>
                    ...
                    <span className={cn(
                      "font-semibold",
                      diferenca === 0 ? "text-green-600" : "text-destructive"
                    )}>
      ...
      {!readonly && (
        <CondicaoPagamentoForm
          ...
          initialData={editingForAdvanced ? {
            tipo_parcela_codigo: editingForAdvanced.tipo_parcela_codigo as TipoParcelaCodigo,
            descricao: editingForAdvanced.descricao || undefined,
            quantidade: editingForAdvanced.quantidade || 1,
